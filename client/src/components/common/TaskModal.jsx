...
const TaskModal = (props) => {
  ...
  const updateAssignedBy = async (event, newValue) => {
    timer = setTimeout(async () => {
      try {
        await taskApi.update(boardId, task.id, { assignedBy: newValue });
      } catch (err) {
        alert(err);
      }
    }, timeout);

    task.assignedBy = newValue;
  ...
  const options = boards?.map(item => item.title)
